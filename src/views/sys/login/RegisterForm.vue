    ...
    <Form class="p-4 enter-x" :model="formData" :rules="getFormRules" ref="formRef">
      ...
      <FormItem name="inviteCode" class="enter-x">
        <Input
          ...
          :placeholder="t('sys.login.inviteCode')"
        />
      </FormItem>
      <FormItem class="enter-x" name="policy">
        <!-- No logic, you need to deal with it yourself -->
        <Checkbox v-model:checked="formData.policy" size="small" />
        {{ t('sys.login.readAndAgree')
        }}<a @click="showView('xieyi')">{{ t('sys.login.userServiceAgreement') }}</a
        ><a @click="showView('yinsi')">{{ t('sys.login.privacyPolicy') }}</a>
      </FormItem>
...
<script lang="ts" setup>
  ...
  import { registerApi, registerByEmailApi } from '/@/api/df/user';
  ...
  onMounted(() => {
    // 获取 URL 参数
    const queryParam = route.query.inviteCode;
    console.log('Query Parameter:', queryParam);
    formData.inviteCode = queryParam;
  ...
  async function showView(viewStr) {
    window.open(viewStr, '_blank');
  }
  ...
  async function sendEmailCode(): Promise<boolean> {
    ...
    createMessage.success('邮件已发送,请去邮箱查看！');
  ...
  async function sendPhoneCode(): Promise<boolean> {
    ...
    // 验证手机号码
    if (!mobile?.trim) {
      createMessage.error('手机号码不能为空');
      return false;
    } else if (!chinaMobileRegex.test(mobile)) {
      createMessage.error('请输入有效的手机号码');
      return false;
    }
    const codeParams: VerifyCodeParams = {
      verifyType: 'P',
  ...
  async function handleRegister() {
    ...
    try {
      ...
      const userInfo = await registerByEmailApi(param);
