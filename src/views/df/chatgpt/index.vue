<template>
  <a-layout v-loading="loadingRef" loading-tip="加载中...">
    <div class="VPFeatures VPHomeFeatures" style="margin-top: 104px">
      <div class="container">
        <div class="VPTeamPageTitle" style="display: flex; justify-content: center">
          <h1 style="margin-bottom: 64px; font-weight: bold"> Midjourney支持程度？ </h1>
        </div>
        <div class="items">
          <div class="item grid-6" v-for="(feature, index) in recordList" :key="index">
            <div data-v-b90cf70b="" class="VPLink no-icon VPFeature">
              <article class="box">
                <!-- <img data-v-8043377a="" class="VPImage" :src="feature.iconSrc" height="48" width="48" alt=""> -->
                <h2 class="title">{{ feature.title }}</h2>
                <p class="details">{{ feature.summary }}</p>
                <!--v-if-->
                <!-- <div data-v-06a12359="" class="link-text">
                <p data-v-06a12359="" class="link-text-value">{{feature.linkText}} 
                  <svg data-v-06a12359="" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="link-text-icon">
...
<script lang="ts" setup>
  ...
  import { allChatgptShare } from '/@/api/df/chatgptShare';
  ...
  onMounted(async () => {
    ...
    try {
      recordList.value = await allChatgptShare();
    } finally {
